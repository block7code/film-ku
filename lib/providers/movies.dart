...
import 'package:film_ku/models/review_model.dart';
import 'package:film_ku/models/video_model.dart';
...
class Movies with ChangeNotifier {
  ...
  Future<void> fetchInTheaters(int page) async {
    ...
    final url = '$BASE_URL/movie/now_playing?api_key=$apiKey&page=$page';
    // '$BASE_URL/movie/now_playing?api_key=${DotEnv().env['API_KEY']}&page=$page';

    try {
      final response = await http.get(url);
      print('pageno yusuf=---------------------> ${url}');
      print('pageno yusuf=---------------------> ${response.body}');
  ...
  Future<void> fetchTrending(int page) async {
    var apiKey = ' ADD YOUR API KEY HERE';
    // '$BASE_URL/movie/now_playing?api_key=$apiKey&page=$page';
    final url =
  ...
  Future<void> fetchUpcoming(int page) async {
    var apiKey = ' ADD YOUR API KEY HERE';
    final url = '$BASE_URL/movie/upcoming?api_key=$apiKey&page=$page';
  ...
  Future<void> fetchForKids(int page) async {
    var apiKey = ' ADD YOUR API KEY HERE';
    final url =
        '$BASE_URL/discover/movie?api_key=$apiKey&language=en-US&sort_by=popularity.desc&page=$page&with_genres=16';
  ...
  Future<void> getMovieDetails(int id) async {
    var apiKey = ' ADD YOUR API KEY HERE';
    final url =
        '$BASE_URL/movie/$id?api_key=$apiKey&language=en-US&append_to_response=credits,similar,reviews,images&include_image_language=en,null';
    try {
      final response = await http.get(url);
      print(' Ci ta cek hinoe  ----------->- ${response.body}');
      final responseData = json.decode(response.body) as Map<String, dynamic>;
      ...
      if (_movieDetails.cast != null) {
        _movieDetails.cast.forEach((element) {
          print('name/ Ci ta cek hinoe ---------> ${element['character']} ');
          cast.add(CastModel(
            id: element['id'],
            name: element['name'],
  ...
  Future<void> fetchVideos(int id) async {
    var apiKey = ' ADD YOUR API KEY HERE';
    final url = '$BASE_URL/movie/$id/videos?api_key=$apiKey&language=en-US';
  ...
  Future<void> getGenre(int id, int page) async {
    ...
    var apiKey = ' ADD YOUR API KEY HERE';
    final url =
        '$BASE_URL/discover/movie?api_key=$apiKey&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=$page&with_genres=$id';
    final response = await http.get(url);
    final responseData = json.decode(response.body) as Map<String, dynamic>;
    final moviesData = responseData['results'];
