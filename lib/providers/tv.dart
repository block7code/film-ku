...
class TV with ChangeNotifier {
  ...
  Future<void> getDetails(int id) async {
    var apiKey = ' ADD YOUR API KEY HERE';
    final url =
        '$BASE_URL/tv/$id?api_key=$apiKey&language=en-US&append_to_response=credits,Cimages,Cvideos,images,similar,reviews&include_image_language=en,null';
    try {
      final response = await http.get(url);
  ...
  Future<void> fetchForKids(int page) async {
    var apiKey = ' ADD YOUR API KEY HERE';
    final url =
        '$BASE_URL/discover/tv?api_key=$apiKey&language=en-US&sort_by=popularity.desc&page=$page&with_genres=10762';
    try {
      final response = await http.get(url);
      // print(response.body);
  ...
  Future<void> getGenre(int id, int page) async {
    var apiKey = ' ADD YOUR API KEY HERE';
    final url =
        '$BASE_URL/discover/tv?api_key=$apiKey&language=en-US&sort_by=popularity.desc&page=$page&with_genres=$id';
    try {
      final response = await http.get(url);
      print(response.body);
